...
example of using rxdb with encryption
...
// Enable dev-mode in development only
...
export type UserDocType = {
  id: string;
  username: string;
  secretNote: string; // will be encrypted
};
...
const userSchema = {
  ...
  required: ["id", "username", "secretNote"],
  // Fields listed here are encrypted at rest; indices cannot be encrypted
  encrypted: ["secretNote"],
} as const;

export async function initDatabase(password: string) {
  // Wrap Dexie storage with RxDB's CryptoJS encryption
  ...
  const db = await createRxDatabase({
    ...
    multiInstance: true, // set false if single tab only
  });
