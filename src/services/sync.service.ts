...
class SyncService {
  ...
  async initForLedger(ledgerId: string, ledgerKey: string) {
    ...
    // Account Groups sync
    const yAccountGroups = this.ydoc.getMap<AccountGroup>("accountGroups");
    yAccountGroups.observe((event) => {
      if (event.transaction.local) return;

      const changesToApply: AccountGroup[] = [];
      event.keysChanged.forEach((key) => {
        const item = yAccountGroups.get(key);
        if (item) changesToApply.push(item);
      });

      if (changesToApply.length > 0) {
        getActiveLedgerDb().accountGroups.bulkPut(changesToApply);
      }
    });

    // Accounts sync
    const yAccounts = this.ydoc.getMap<Account>("accounts");
    yAccounts.observe((event) => {
      if (event.transaction.local) return;

      const changesToApply: Account[] = [];
      event.keysChanged.forEach((key) => {
        const item = yAccounts.get(key);
        if (item) changesToApply.push(item);
      });

      if (changesToApply.length > 0) {
        getActiveLedgerDb().accounts.bulkPut(changesToApply);
      }
    });
  }
  ...
  pushAccountGroup(accountGroup: AccountGroup) {
    if (!this.ydoc) return;
    const yAccountGroups = this.ydoc.getMap<AccountGroup>("accountGroups");
    yAccountGroups.set(accountGroup.id, accountGroup);
  }

  pushAccount(account: Account) {
    if (!this.ydoc) return;
    const yAccounts = this.ydoc.getMap<Account>("accounts");
    yAccounts.set(account.id, account);
  }
