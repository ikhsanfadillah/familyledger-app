...
import { FieldInput } from "@ark-ui/solid/field";
...
export interface InputProps
  extends Omit<ComponentProps<typeof FieldInput>, "size">, VariantProps<typeof inputVariants> {}

export const Input = (props: InputProps) => {
  // const { size = "md", type = "text", className, ...rest } = props;
  const [local, rest] = splitProps(
    mergeProps({ size: "md" as const, type: "text" as const }, props),
    ["size", "type"],
  );

  return (
    <FieldInput
      class={cn(inputVariants({ size: local.size }), rest.class)}
